...
def get_random_chunk(split):
    ...
    if not os.path.exists(filename):
        # Fallback dummy text if corpus files aren't found yet
        return torch.tensor(encode("Hello world! This is a BPE + MoE hybrid model test."), dtype=torch.long)

    with open(filename, 'rb') as f:
        with mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ) as mm:
            file_size = len(mm)
            # Ensure we read enough bytes for tokens
...
def get_batch(split):
    ...
    if len(data_chunk) <= block_size:
        data_chunk = torch.tensor(encode("Fallback training sentence for BPE MoE model."), dtype=torch.long)
...
print("Initializing BPE + MoE Model...")
model = BPEMoELanguageModel(vocab_size).to(device)
...
with open("bpe_moe_model.pk1", "wb") as f:
    pickle.dump(model, f)
print("Training complete! Model saved to bpe_moe_model.pk1")
