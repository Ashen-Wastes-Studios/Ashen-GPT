...
class AshenAIAgenticEngine:
    ...
    def classify_input(self, prompt):
        try:
            ...
            # once before giving up. The extra activations from a prior
            # generation may not have been freed yet.
            msg = str(e)
    ...
    def solve_with_agent_stream(self, prompt):
        ...
        # events in the same schema so the CLI renders it like any other turn.
        if _is_illegal_request(prompt):
            _thought, _resp = LEGAL_REFUSAL_THOUGHT, LEGAL_REFUSAL_RESPONSE
            self.last_sources = []
            self.history.append((prompt, f"<think>\n{_thought}\n</think>\n{_resp}"))
            yield {"type": "thought_delta", "chunk": _thought}
            yield {"type": "thought_done"}
            yield {"type": "response_delta", "chunk": _resp}
            yield {"type": "done", "thought": _thought, "response": _resp,
                   "model": os.path.basename(current_model_filename),
